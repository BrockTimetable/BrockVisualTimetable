  ...
  timetable.courses.forEach((course) => {
      ...
      mainComponents.forEach((mainComponent) => {
        ...
        if (!mainComponent.schedule.time) {
          ...
          if (/^\d{3,4}-\d{3,4}$/.test(cleanTime)) {
            ...
            modifiedComponent.schedule = {
              ...mainComponent.schedule,
              time: cleanTime,
            };
            // Ensure days are properly formatted
            if (
              modifiedComponent.schedule.days &&
              !modifiedComponent.schedule.days.includes(" ")
            ) {
              modifiedComponent.schedule.days = modifiedComponent.schedule.days
                .split("")
                .join(" ");
            }
            ...
            if (timeMatch) {
              ...
              if (
                ...
                modifiedComponent.schedule.days =
                  modifiedComponent.schedule.days.split("").join(" ");
              }
              addTimedEvent(course, modifiedComponent, "default", courseColors);
            } else {
              addAllDayEvent(course, mainComponent, "default", courseColors);
    ...
    if (secondaryComponents) {
      ...
      const handleComponent = (component) => {
        ...
        const isSynchronousOnline =
          component.type.startsWith("SYN") || component.type.startsWith("HYF");
