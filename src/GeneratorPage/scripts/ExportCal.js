...
function generateICSFileData() {
    ...
    for (let i = 0; i < courses.length; i++) {
        ...
        for (let j = 0; j < mainComponents.length; j++){
            ...
            if (!component.schedule.time || component.schedule.time == "Project Course"){
                continue;
            }
            ICSData = ICSData + addComponent(component, courseCode);
        }

        let secondaryComponents = cachedTimetableData.courses[i].secondaryComponents;
...
function generateICSEvent(startTimestamp, endTimestamp, endReoccurTimestamp, days, title, description) {
    const dayICSMap = {
        U: "SU", M: "MO", T: "TU", W: "WE", R: "TH", F: "FR", S: "SA"
    }
    let EventData = "BEGIN:VEVENT\n"
    EventData = EventData + "UID:" + self.crypto.randomUUID() + "\n";
    EventData = EventData + "DTSTAMP:" + generateICSTimeStampFromDate(new Date()) + "\n";
    EventData = EventData + "DTSTART:" + generateICSTimeStampFromDate(new Date(startTimestamp * 1000)) + "\n";
    EventData = EventData + "DTEND:" + generateICSTimeStampFromDate(new Date(endTimestamp * 1000)) + "\n";
    EventData = EventData + "RRULE:FREQ=WEEKLY;" + "UNTIL=" + generateICSTimeStampFromDate(new Date(endReoccurTimestamp * 1000)) + ";BYDAY=";
...
function addComponent(component, courseCode){
    ...
    const firststartTimestamp = findPreciseEventTimestamp(component.schedule.startDate, componentdaysArray, startTime);
    const firstendTimestamp = findPreciseEventTimestamp(component.schedule.startDate, componentdaysArray, endTime);
    return generateICSEvent(firststartTimestamp, firstendTimestamp, component.schedule.endDate, componentdaysArray, courseCode + " " + component.type, component.instructor);
}
