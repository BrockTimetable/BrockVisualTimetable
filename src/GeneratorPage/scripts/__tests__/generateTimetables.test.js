...
import { generateTimetables, getValidTimetables, getGenerationPerformance } from '../generateTimetables';
...
describe('Timetable Generation', () => {
    ...
    it('should track performance metrics correctly', () => {
        // Mock course data with multiple sections to generate significant combinations
        const mockCourseData = {
            'COSC 1P02': {
                courseCode: 'COSC 1P02',
                sections: Array(5).fill(0).map((_, i) => ({
                    id: `${i}`,
                    schedule: {
                        days: 'M W',
                        time: '0800-0920',
        ...
        expect(metrics).toHaveProperty('totalCombinationsProcessed');
        expect(metrics).toHaveProperty('validTimetablesFound');
        expect(metrics).toHaveProperty('timeSlotOverrides');
        expect(metrics).toHaveProperty('combinationsPerSecond');

        // Verify metrics have reasonable values
        expect(parseFloat(metrics.totalTime)).toBeGreaterThan(0);
        expect(metrics.totalCombinationsProcessed).toBeGreaterThan(0);
        expect(metrics.validTimetablesFound).toBeGreaterThan(0);
        expect(metrics.combinationsPerSecond).toBeGreaterThan(0);
    });
}); 