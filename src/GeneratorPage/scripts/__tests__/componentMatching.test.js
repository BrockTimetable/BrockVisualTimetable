...
describe("Component Matching and Filtering", () => {
  ...
  it("should filter components by duration when duration is pinned", () => {
    ...
    const validTimetables = timetables.filter((timetable) => {
      ...
      const mainComponentsValid = course.mainComponents.every(
        (component) => component.schedule.duration === "D2",
      );
    ...
    expect(
      validTimetables.every((timetable) => {
        const course = timetable.courses[0];
  ...
  it("should match secondary components with their correct main sections", () => {
    ...
    expect(
      timetables.every((timetable) => {
        ...
        return (
          lab.id.charAt(getMatchingCharIndex(lab.id)) ===
          mainSection.id.charAt(getMatchingCharIndex(mainSection.id))
        );
      }),
    ).toBe(true);
  });
